...
proc do_placement {place_density global_placement_args} {
  ...
  if { 0 != [llength [array get ::env GLOBAL_PLACEMENT_ARGS]] } {
    ...
    if {[info exist ::env(PL_BASIC_PLACEMENT)] } {
        lappend all_args -overflow 0.9
        lappend all_args -init_density_penalty 0.0001
        lappend all_args -initial_place_max_iter 20
        lappend all_args -bin_grid_count 64
    }
...
if {[info exist ::env(TSV_SET_SCRIPT)]} {
    source $::env(TSV_SET_SCRIPT)
}

if {[info exist ::env(CELL_SET_SCRIPT)]} {
    source $::env(CELL_SET_SCRIPT)
}
